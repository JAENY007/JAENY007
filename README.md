# Front-End Developer, 전재연입니다

![https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fjaeny007.notion.site%2FFront-End-Developer-8ad53d74fe9746bebd78c13ad3752deb&count_bg=%235AA3FC&title_bg=%23555555&icon=&icon_color=%23FFFFFF&title=hits&edge_flat=false](https://hits.seeyoufarm.com/api/count/incr/badge.svg?url=https%3A%2F%2Fjaeny007.notion.site%2FFront-End-Developer-8ad53d74fe9746bebd78c13ad3752deb&count_bg=%235AA3FC&title_bg=%23555555&icon=&icon_color=%23FFFFFF&title=hits&edge_flat=false)

# 🌐 **Contact & Channel**

![KakaoTalk_20220104_164631488.jpg](Front-End%20%20eda36/KakaoTalk_20220104_164631488.jpg)

✉️ **Email** | 

**wjswodus22@naver.com**

🚀 **Github** |

[**https://github.com/JAENY007**](https://github.com/JAENY007)

🪶 **Blog** |

[**https://developer-jaeny.tistory.com/**](https://developer-jaeny.tistory.com/)

📷 **Instagram** |

[**https://www.instagram.com/choco_jaeny**](https://www.instagram.com/choco_jaeny)

# 🧑🏻‍🦰 **Introduction**

---

## About me

- 안녕하세요! 신입 프론트 엔드 개발자 전재연입니다
    
    개발자라는 꿈을 갖기 이전에는 평범한 직장인으로 살다가, 몰두하는 즐거움을 알게되어 개발에 빠지게 되었습니다.
    
    그 중에서도 User와 더 가까운 프론트 엔드의 매력에 빠져 좀 더 아름다우면서도 User 친화적인 개발을 하고 싶다는 꿈이 생겼습니다
    
- 새로운 기술을 배우는데에 거부감이 없으며, 조금이라도 관심이 있다면 더욱 몰두하는 것을 좋아합니다
- 완벽을 추구하는 성향이 있어, 세심한 부분도 놓치지 않도록 코드 리팩토링을 확실히 진행하겠습니다
- 적극적이고 지속적인 Retrospect을 통해 발전하겠습니다
- 지속적인 개발, 꾸준함을 위해 공부를 게을리 하지 않겠습니다
- 원만한 소통과 협업이 가능한 개발자가 되겠습니다

# ⚙️ **Skills**

**Green** - Usable / **Purple** - Experienced

---

## Front-End

### **HTML, CSS, JavaScript**

## Back-End

### **Java, Python**

## DataBase

### **Oracle**

## Framework

### **Spring MVC, Spring Boot**

## Library

### **React**

## Tools & Collaboration

**VScode, Github, Notion, Slack, Gather**

**Eclipse, STS, IntelliJ, PyCharm, Spyder, Jupyter Notebook, Postman,**

**Trello**

# 👥 Experienced & Projects

---

## **Fit Box**

### 운동 종합정보 플랫폼

22. 03. 07 ~ 22. 03. 25

[[FIT BOX] - Final Project](https://periodic-slice-dd0.notion.site/FIT-BOX-Final-Project-13e35063a9454e00b20fd8e616b483b1)

[Encore-FinalProject-Team3](https://github.com/Encore-FinalProject-Team3)

- Front-End 전반 개발 담당
    - 기획 과정 참여
        - 화면정의서
        - 요구사항 정의서
        - DB설계
        - 페이지 설계
        - 스프린트 계획
            
            등 기획 단계를 체계적으로 하여 프로젝트 개발에 변경 사항을 최대한 줄일 수 있었음
            
    - JavaScript와 React를 이용한 페이지 작성
        - 4~5일 정도의 Study 후 기술 적용
        - 급하게 공부하고 적용하는데에 어려움이 있었음
        - 담당 멘토를 통한 멘토링으로 최대한 어려움을 극복
    - Axios 기술을 통한 API 연동
        - 아직 이해도가 부족하나, 더 공부할 예정
    
    + 리팩토링 혹은 전체 리빌딩 이후,
    
    가능하면 백엔드 팀원들과 함께 배포예정
    

**발표자료 작성**

[FitBox_PPT_작성자_전재연.pdf](Front-End%20%20eda36/FitBox_PPT_%EC%9E%91%EC%84%B1%EC%9E%90_%EC%A0%84%EC%9E%AC%EC%97%B0.pdf)

---

## 코로나 19와 서울시 상권의 상관관계 분석

Python을 활용한 데이터 분석 프로젝트

21. 12. 14 ~ 21. 12. 30

- 문서 작성 담당
    - Python이나 데이터 분석 쪽 공부가 부족하여 각종 문서 작성 등을 위주로 프로젝트 참여
    - Python이나 각종 라이브러리 활용은 어려웠지만, 데이터 분석의 흐름을 어느정도 파악할 수 있는 계기가 됨
    - 추후 Python 을 다시 공부할 예정

**발표자료 작성**

[211230_TeamPJT_0.pdf](Front-End%20%20eda36/211230_TeamPJT_0.pdf)

---

## Travel

게시판 커뮤니티 만들기 팀 프로젝트

21. 11. 22 ~ 21. 12. 03

[](https://github.com/EncoreTeam3/GARBAGE)

- MyBatis, Session 통신 위주
HTML, CSS, Thymeleaf를 사용한
레거시 Spring Boot Project

- Front-End 담당
    - HTML, CSS를 활용한 간단한 페이지 작성
    - 로그인, 게시글 목록 부분 화면 HTML, CSS 작성
    - 이후 해당 내용(프론트)을 더 공부하기로 결심하고, 혼자 공부를 시작하게 되었음

**프로젝트 발표 자료**

[미니프로젝트2.pdf](Front-End%20%20eda36/%EB%AF%B8%EB%8B%88%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B82.pdf)

---

## Petstagram

게시판 만들기 튜토리얼

21. 11. 08 ~ 21. 11. 11

- Java, Spring, HTML, CSS를 이용한 튜토리얼
    
    미니 프로젝트
    
    - 과정 중에 배운 HTML, CSS 지식이 부족하여,
        
        혼자서 공부하면서 채우다 보니 각종 태그나
        
        CSS 선택자나 속성 등 사용의 어려움이 존재
        
        공식문서 등을 찾아보면서 최대한 채우려고
        
        노력하였음
        

**발표자료 작성**

[Petstargram_PT.pdf](Front-End%20%20eda36/Petstargram_PT.pdf)

# 📖 Education

---

## 일산대진고등학교

**졸업**

2009. 02 ~ 2012. 02

- 이과 졸업(예체능 희망)

## 엔코아 플레이데이터

**인공지능 16기 과정 수료**

2021. 09. 15 ~ 2022. 03. 26

- 엔코아 플레이데이터에서 진행한 인공지능 개발자 양성 교육과정
    - Java, Oracle, Spring, Python, OpenCV 등을 통해 배우는 인공지능 관련 교육과정
    - 개인 및 팀 단위의 프로젝트를 통한 업무 경험
    - 과정 진행 중, 백엔드 업무보다 프론트 엔드가 성향에 적합하다 판단하여 개인적으로 공부하기 시작함

# 🎡 Activities

---

지속적인 성장을 위해 꾸준히 스스로를 개발하는 개발자가 되겠습니다.

### 몇 권이든 읽어보자,

개발서적 포함 책 읽기 프로젝트

[RB(Reading Books) Project](https://jaeny007.notion.site/RB-Reading-Books-Project-9b24901e1b6c44d6bdb3aa2272f8e813)

### 회고하는 능력을 키우기 위해,

Daily Diary 작성

객관적인 생각과 리뷰를 위해

하루 업무 등에 대한 Retrospect Diary 작성 예정

### 필요한 공부를 찾아서,

필요한 언어 등 인터넷 수강하기

Code it 기존 수강 중

(충분한 수양 이후 Side Project 진행 예정)
